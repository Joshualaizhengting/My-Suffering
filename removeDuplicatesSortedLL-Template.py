...
def remove_duplicates_sorted_ll(ll):
    ...
    if ll.head is None or ll.head.next is None:
        return
    #idea is the same as maxval => we store a temp val for each element and compare them with the next val if next val not equal then we move on if it is then we delete that node
    curr = ll.head
